  ...
  var Drawer = /*#__PURE__*/function () {
    ...
    _createClass(Drawer, [{
      ...
      value: function open(event) {
        ...
        // 編集　トップヘッダーとブランドヘッダーによって出し分け
        // URLの取得
        window.onload = ()=> {
          let page_url = location.pathname;
          if (page_url == "/pages/hibrow" || "/pages/hellos" || "/products" || "/collections") {
            document.querySelector('#shopify-section-header-custom').style.zIndex = ''; // ブランドトップのヘッダー
          }else{
            document.querySelector('#shopify-section-header').style.zIndex = ''; // トップのヘッダー
          }
        }
          // document.querySelector('#shopify-section-header').style.zIndex = ''; // Ugly hack　元々のコード
    ...
    _createClass(SearchBar, [{
      ...
      value: function _openSearch() {
        ...
        window.onload = ()=> {
          let page_url = location.pathname;
          if (page_url == "/pages/hibrow" || "/pages/hellos" || "/products" || "/collections") {
            document.querySelector('#shopify-section-header-custom').style.zIndex = 10; // ブランドトップのヘッダー
          }else{
            document.querySelector('#shopify-section-header').style.zIndex = 10; // トップのヘッダー
          }
        }
      }
      /**
